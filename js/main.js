...
const updatePersistentData = (listArray) => {
  // Parse items with private fields
  localStorage.setItem("myToDoList", JSON.stringify(listArray.toPublicList()));
};
...
const calcNextItemId = () => {
  ...
  const list = toDoList.list;
  if (list.length > 0) {
    nextItemId = list[list.length - 1].id + 1;
  }
  return nextItemId;
};
...
const updateScreenReaderConfirmation = (entryText, actionVerb) => {
  document.getElementById(
    "confirmation"
  ).textContent = `${entryText} ${actionVerb}.`;
};
